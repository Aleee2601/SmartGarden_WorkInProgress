...
import {
  ...
  Info, Home, Plus, Check, X, Play, AlertCircle, Loader
} from 'lucide-react';
...
function App() {
  ...
  const [calibrationData, setCalibrationData] = useState({
    lightSensor: {
      dark: { calibrated: false, value: null },
      lowLight: { calibrated: false, value: null },
      mediumIndirect: { calibrated: false, value: null },
    ...
    <div className="bg-white rounded-3xl shadow-xl p-8 space-y-6">
      ...
      <div className="space-y-4">
        ...
        <button
          onClick={handleLogin}
          disabled={loading}
          className="w-full py-3 bg-gradient-to-r from-green-500 to-green-600 text-white rounded-lg font-semibold hover:from-green-600 hover:to-green-700 transition-all transform hover:scale-105 disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center"
        >
          {loading ? (
            <>
...
function SignUpScreen({ onNavigate, onSignup }) {
  ...
  const handleSignUp = async () => {
    ...
    try {
      ...
      onSignup(response.user || response);
    } catch (err) {
    ...
    <div className="bg-white rounded-3xl shadow-xl p-8 space-y-6">
      ...
      <div className="space-y-4">
        ...
        <button
          ...
          disabled={loading}
          className="w-full py-3 bg-gradient-to-r from-green-500 to-green-600 text-white rounded-lg font-semibold hover:from-green-600 hover:to-green-700 transition-all disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center"
        >
          {loading ? (
            <>
              <Loader className="w-5 h-5 mr-2 animate-spin" />
...
function CalibrationScreen({ onNavigate, calibrationData, setCalibrationData, selectedPlant }) {
  ...
  const startCalibration = async (sensorType, stepKey, instruction) => {
    // Enable calibration mode on backend if plant is selected
    if (selectedPlant) {
      try {
        await plantService.setCalibrationMode(selectedPlant.plantId, true);
      } catch (error) {
        console.error('Failed to enable calibration mode:', error);
      }
    }
  ...
  const saveCalibrationData = async () => {
    ...
    try {
      await plantService.updateCalibration(selectedPlant.plantId, calibrationData);
      await plantService.setCalibrationMode(selectedPlant.plantId, false);
      onNavigate('dashboard');
    } catch (error) {
      console.error('Failed to save calibration:', error);
      alert('Failed to save calibration data. Please try again.');
    } finally {
      setSaving(false);
    }
  };
      ...
      <div className="p-6 border-t flex justify-between">
        ...
        {isAllCalibrated() && (
          <button
            ...
            disabled={saving}
            className="px-6 py-3 bg-gradient-to-r from-green-500 to-green-600 text-white rounded-lg hover:from-green-600 hover:to-green-700 transition-all flex items-center space-x-2 disabled:opacity-50"
          >
            {saving ? (
              <>
...
function CalibrationModal({ sensorType, stepKey, instruction, selectedPlant, onComplete, onCancel }) {
  const [countdown, setCountdown] = useState(ENV.SENSORS.CALIBRATION_COUNTDOWN);
  ...
  useEffect(() => {
    if (isCalibrating && countdown > 0) {
      ...
      // Fetch latest sensor reading if plant is selected
      if (selectedPlant && latestReading) {
        const sensorField = getSensorField(sensorType);
        const value = latestReading[sensorField] || Math.floor(Math.random() * 1000);
        onComplete(value);
      } else {
        // Generate random calibration value for demo
        const randomValue = Math.floor(Math.random() * 1000);
        onComplete(randomValue);
      }
    }
  }, [isCalibrating, countdown, onComplete, sensorType, selectedPlant, latestReading]);

  const getSensorField = (type) => {
    switch (type) {
      case 'light': return 'lightLevel';
      case 'soilMoisture': return 'soilMoisture';
      case 'waterLevel': return 'waterLevel';
      case 'temperature': return 'airTemperature';
      default: return 'lightLevel';
    }
  };

  const startCalibrating = async () => {
    // Fetch latest sensor reading if plant is selected
    if (selectedPlant) {
      try {
        const reading = await sensorService.getLatestReading(selectedPlant.plantId);
              ...
              <div className="relative w-32 h-32">
                <svg className="transform -rotate-90 w-32 h-32">
                  ...
                  <circle
                    ...
                    strokeDashoffset={`${2 * Math.PI * 56 * (countdown / ENV.SENSORS.CALIBRATION_COUNTDOWN)}`}
...
// Dashboard Screen Component (Part 1)
function DashboardScreen({ user, onNavigate, onPlantSelect, onLogout, showMenu, setShowMenu }) {
  ...
  const handleApproveDevice = async (deviceId) => {
    try {
      ...
      const devicesData = await deviceService.getPendingDevices();
      setPendingDevices(devicesData);
    } catch (err) {
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center space-x-3">
            ...
            <div className="flex items-center space-x-3">
              ...
              <div>
                <p className="text-white font-semibold">{user?.username || user?.email || 'User'}</p>
      ...
      {showMenu && (
        <div className="fixed inset-0 bg-black bg-opacity-50 z-50" onClick={() => setShowMenu(false)}>
          ...
          >
            ...
            <button
              onClick={onLogout}
              className="w-full flex items-center space-x-3 p-3 hover:bg-red-100 rounded-lg transition-all text-red-600"
            >
              <X className="w-5 h-5" />
              <span>Logout</span>
            </button>
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-2">
              ...
              <span className="text-sm font-semibold text-orange-800">
                {pendingDevices.length} device(s) pending approval
              </span>
            </div>
            <button
              onClick={() => handleApproveDevice(pendingDevices[0].deviceId)}
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex items-center space-x-4">
                      ...
                      <div>
                        ...
                        <p className="text-green-100 text-sm italic">{plant.species}</p>
...
function PlantDetailScreen({ plant, onNavigate }) {
  const [autoWatering, setAutoWatering] = useState(plant?.autoWateringEnabled || false);
  ...
  useEffect(() => {
    if (plant) {
      ...
      const stopPolling = sensorService.startPolling(plant.plantId, (data) => {
        setSensorData(data);
      });
  ...
  const fetchSensorData = async () => {
    try {
      const data = await sensorService.getLatestReading(plant.plantId);
      setSensorData(data);
    } catch (error) {
  ...
  const fetchWeeklyData = async () => {
    try {
      const data = await sensorService.getWeeklyStatistics(plant.plantId);
      setWeeklyData(data);
    } catch (error) {
  ...
  const handleToggleAutoWatering = async (enabled) => {
    try {
      await wateringService.configureAutoWatering(plant.plantId, {
        enabled,
        intensity: wateringIntensity
      });
      setAutoWatering(enabled);
    } catch (error) {
      alert('Failed to update auto-watering: ' + error.message);
    }
  };
  ...
  const handleManualWater = async () => {
    setWatering(true);
    try {
      await wateringService.waterManually(plant.plantId, 30);
      alert('Watering started for 30 seconds');
    } catch (error) {
      alert('Failed to start watering: ' + error.message);
    } finally {
      setWatering(false);
    }
  };
  ...
  const sensorReadings = [
    ...
    { label: 'Temperature', value: sensorData?.airTemperature || 0, unit: '°C', icon: ThermometerSun, warning: false },
    { label: 'Soil Moisture', value: sensorData?.soilMoisture || 0, unit: '%', icon: Droplets, warning: false }
  ];
      ...
      <div className="bg-gradient-to-br from-green-400 to-green-600 p-6">
        ...
        <div className="flex items-start space-x-4">
          ...
          <div className="flex-1 text-white">
            ...
            <p className="text-green-100 italic">{plant.species}</p>
            {sensorData && (
              <div className="mt-3 flex items-center space-x-4 text-sm">
                ...
                <div className="flex items-center space-x-1">
                  ...
                  <span>{sensorData.lightLevel} lux</span>
                </div>
              </div>
            )}
          </div>
          ...
          <div className="p-6 space-y-4">
            <div className="flex items-center justify-between">
              ...
              <button
                ...
                disabled={watering}
                className="px-4 py-2 bg-blue-500 text-white rounded-lg text-sm hover:bg-blue-600 disabled:opacity-50 flex items-center space-x-2"
              >
                {watering ? (
                  <>
          ...
          <div className="p-6 space-y-4 border-t">
            ...
            <div className="flex items-center justify-between p-4 bg-gray-50 rounded-xl">
              ...
              <div className="flex items-center space-x-2 bg-white rounded-full p-1 shadow-inner">
                <button
                  onClick={() => handleToggleAutoWatering(false)}
                ...
                <button
                  onClick={() => handleToggleAutoWatering(true)}
                  className={`px-4 py-2 rounded-full text-sm font-semibold transition-all ${
                  ...
                  <div key={index} className="flex-1 flex flex-col items-center space-y-1">
                    <div className="w-full bg-gray-200 rounded-t-lg relative" style={{ height: '150px' }}>
                      <div
                        ...
                        style={{ height: `${Math.min((day.light / 100) * 150, 150)}px` }}
                      />
                      <div
                        className="absolute bottom-0 w-full bg-blue-400"
                        style={{ height: `${Math.min((day.water / 100) * 150, 150)}px` }}
                      />
                      <div
                        className="absolute bottom-0 w-full bg-orange-400"
                        style={{ height: `${Math.min((day.temp / 100) * 150, 150)}px` }}
                      />
                      <div
                        className="absolute bottom-0 w-full bg-amber-600"
                        style={{ height: `${Math.min((day.soil / 100) * 150, 150)}px` }}
                      />
                    </div>
